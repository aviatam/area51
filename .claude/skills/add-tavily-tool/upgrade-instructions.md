...
2. Open this link and paste the key into the prefilled form, then save:
   ...
   (That is the OneCLI dashboard on the Area51 host; the key goes straight
...
```bash
area51 groups config remove-mcp-server --name tavily
area51 groups config add-mcp-server --name tavily \
  --command mcp-remote \
  --args '["https://mcp.tavily.com/mcp/","--transport","http-only","--enable-proxy","--header","X-Client-Name:area51","--ignore-tool","tavily_crawl","--ignore-tool","tavily_map","--ignore-tool","tavily_research"]' \
  --env '{}'
