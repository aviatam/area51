...
test('/api/help: returns raw per-resource help text', async () => {
  await withServer({ area51: cli() }, async (base) => {
    const body = await (await fetch(`${base}/api/help/area51/sessions`)).json();
...
test('/api/help: undiscovered resource → 404', async () => {
  await withServer({ area51: cli() }, async (base) => {
    assert.equal((await fetch(`${base}/api/help/area51/evil`)).status, 404);
  });
});
...
test('/api/help: a cli without a help template → 404', async () => {
  ...
  await withServer({ area51: c }, async (base) => {
    assert.equal((await fetch(`${base}/api/help/area51/sessions`)).status, 404);
  });
});
...
test('/api/clis: reports help availability per cli', async () => {
  ...
  await withServer({ area51: cli(), docker: noHelp }, async (base) => {
    const body = await (await fetch(`${base}/api/clis`)).json();
    assert.equal(body.clis.find((c) => c.name === 'area51').help, true);
