...
In `src/container-runner.ts`, after the `AREA51_MCP_SERVERS` block, add:
...
CTR=$(docker ps --filter "name=area51-v2-<FOLDER>" --format "{{.Names}}" | head -1)
