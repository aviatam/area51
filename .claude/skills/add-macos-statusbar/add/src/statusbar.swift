...
class StatusBarController: NSObject {
    ...
    private let plistPath = "\(NSHomeDirectory())/Library/LaunchAgents/com.area51.plist"

    /// Derive the Area51 project root from the binary location.
    ...
    private func setupStatusItem() {
        ...
        if let button = statusItem.button {
            if let image = NSImage(systemSymbolName: "bolt.fill", accessibilityDescription: "Area51") {
                image.isTemplate = true
                button.image = image
            } else {
                button.title = "⚡"
            }
            button.toolTip = "Area51"
        }
    }
    ...
    private func checkRunning() -> Bool {
        ...
        task.arguments = ["list", "com.area51"]
        let pipe = Pipe()
    ...
    private func updateMenu() {
        ...
        let label = isRunning ? "Area51 is running" : "Area51 is stopped"
    ...
    @objc private func restartService() {
        let uid = getuid()
        run("/bin/launchctl", ["kickstart", "-k", "gui/\(uid)/com.area51"])
        refresh(after: 3)
    }

    @objc private func viewLogs() {
        let logPath = "\(StatusBarController.projectRoot)/logs/area51.log"
