...
description: Add a macOS menu bar status indicator for Area51. Shows a bolt icon with a green/red dot indicating whether Area51 is running, with Start, Stop, and Restart controls. macOS only.
---

# Add macOS Menu Bar Status Indicator

Adds a persistent menu bar icon that shows Area51's running status and lets the user
...
```bash
launchctl list | grep com.area51.statusbar
```
...
Create `~/Library/LaunchAgents/com.area51.statusbar.plist`, substituting the actual values
...
<dict>
    <key>Label</key>
    <string>com.area51.statusbar</string>
...
```bash
launchctl load ~/Library/LaunchAgents/com.area51.statusbar.plist
```
...
> The bolt icon should now appear in your macOS menu bar. Click it to see Area51's status and control the service.
>
> - **Green dot** — Area51 is running
> - **Red dot** — Area51 is stopped
>
