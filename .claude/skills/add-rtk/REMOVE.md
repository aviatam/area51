...
Idempotent — safe to run even if some steps were never applied. Run Steps 1–3 once per agent group that had rtk wired (`area51 groups list`).
...
```bash
area51 groups config remove-mount --id <group-id> \
  --host ~/.local/bin/rtk \
