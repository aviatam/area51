...
In `container/agent-runner/src/index.ts`, remove the `ollama: { … }` entry from the `mcpServers` object (leave `area51` and any other entries).
...
```bash
grep "\[OLLAMA\]" logs/area51.log | tail -5
```
