...
export async function run(_args: string[]): Promise<void> {
  ...
  if ((process.env.AREA51_RUNTIME_BACKEND || '').toLowerCase() === 'incus') {
    try {
      execSync('incus version', { stdio: 'ignore' });
      containerRuntime = 'incus';
    } catch {
      containerRuntime = 'incus_unavailable';
    }
  }
  ...
  if (fs.existsSync(path.join(homeDir, '.config', 'area51', 'mount-allowlist.json'))) {
