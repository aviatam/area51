...
#
# Area51 needs `signal-cli` on PATH to talk to Signal. Picks the right
...
#
# Emits the standard Area51 STATUS block on success or failure so the
...
emit_status() {
  ...
  echo "=== AREA51 SETUP: INSTALL_SIGNAL_CLI ==="
  echo "STATUS: ${status}"
