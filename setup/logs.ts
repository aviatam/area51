 ...
 *     rawLog);
 *
 * area51.sh emits the bootstrap entry directly via a bash helper so
...
// a AREA51_SKIP list when re-executing after a Claude-assisted fix, so
...
/** Wipe prior logs and write a header. Called once per fresh run (by area51.sh or as a fallback by auto.ts if invoked standalone). */
 ...
 * pre-existing 01-bootstrap.log (written by area51.sh before this module
