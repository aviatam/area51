...
describe('plist generation', () => {
  it('contains the slug-scoped label', () => {
    const projectRoot = '/home/user/area51';
    ...
    expect(plist).toMatch(/<string>com\.area51-v2-[0-9a-f]{8}<\/string>/);
  });
  ...
  it('points to dist/index.js', () => {
    ...
    expect(plist).toContain('/home/user/area51/dist/index.js');
  });
  ...
  it('sets log paths', () => {
    ...
    expect(plist).toContain('area51.log');
    expect(plist).toContain('area51.error.log');
  });
});
...
describe('systemd unit generation', () => {
  ...
  it('sets correct ExecStart', () => {
    ...
    expect(unit).toContain(
      'ExecStart=/usr/bin/node /srv/area51/dist/index.js',
    );
  });
});
...
nohup ${JSON.stringify(nodePath)} ${JSON.stringify(projectRoot)}/dist/index.js >> ${JSON.stringify(projectRoot)}/logs/area51.log 2>> ${JSON.stringify(projectRoot)}/logs/area51.error.log &
echo $! > ${JSON.stringify(pidFile)}`;

    expect(wrapper).toContain('#!/bin/bash');
    expect(wrapper).toContain('nohup');
    expect(wrapper).toContain(nodePath);
    expect(wrapper).toContain('area51.pid');
  });
});
