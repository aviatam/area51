 ...
 *   key 'fooBar' → envVar 'AREA51_FOO_BAR' → flag '--foo-bar'
 *
 * Surface levels:
...
interface BaseEntry {
  ...
  key: string;
  /** Override of the auto-derived AREA51_<UPPER_SNAKE> env var. */
  envVar?: string;
...
export const CONFIG: Entry[] = [
  ...
  {
    key: 'assistMode',
    envVar: 'AREA51_SETUP_ASSIST_MODE',
    label: 'Assist mode',
  ...
  // Uninstall route — handled in auto.ts before any setup work begins.
  {
    key: 'uninstall',
    label: 'Uninstall',
    help: 'Remove this Area51 copy (service, containers, data, vault agents). Asks per group.',
    surface: 'flag',
    type: 'boolean',
    default: false,
  },
...
export function envVarFor(e: Entry): string {
  ...
  return `AREA51_${e.key.replace(/[A-Z]/g, (c) => `_${c}`).toUpperCase()}`;
}
