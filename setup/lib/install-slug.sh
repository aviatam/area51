...
#   label=$(launchd_label)        # com.area51-v2-<slug>
#   unit=$(systemd_unit)          # area51-v2-<slug>
#   image=$(container_image_base) # area51-agent-v2-<slug>
#
# Slug is sha1(PROJECT_ROOT)[:8] — must match the TS helper exactly so both
# halves of setup name things consistently.

_area51_install_slug() {
...
launchd_label() {
  printf 'com.area51-v2-%s' "$(_area51_install_slug)"
}

systemd_unit() {
  printf 'area51-v2-%s' "$(_area51_install_slug)"
}

container_image_base() {
  printf 'area51-agent-v2-%s' "$(_area51_install_slug)"
}
