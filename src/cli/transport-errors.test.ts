...
describe('formatTransportError', () => {
  it('renders per-install service names on ENOENT, not the bare v1 names', () => {
    const out = formatTransportError(new Error('connect ENOENT /tmp/area51.sock'));

    // Regression for #2484: pre-fix, this string was a hardcoded
    // `com.area51` / `area51`, which doesn't match the actual
    ...
    expect(out).not.toMatch(/gui\/\$\(id -u\)\/com\.area51\b(?!-v2)/);
    expect(out).not.toMatch(/systemctl --user restart area51\b(?!-v2)/);
  });
  ...
  it('falls back to a generic transport error for other failures', () => {
    ...
    expect(out).toBe('area51: transport error: some unrelated failure\n');
