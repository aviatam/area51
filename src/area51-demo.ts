...
export async function runArea51Demo(options: Area51DemoOptions = {}): Promise<Area51DemoReport> {
  ...
  fs.writeFileSync(path.join(demoDir, 'reports', 'incus-runtime-plan.json'), JSON.stringify(incusRuntimePlan, null, 2) + '\n');
  ...
  const report: Area51DemoReport = {
    ...
    verified: {
      ...
      quarantine_artifacts_written: Boolean(agentGateReport.quarantine.path && agentGateReport.quarantine.files.length > 0),
