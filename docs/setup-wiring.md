...
2. **Per-wiring override** — the explicit value chosen at creation (`area51` flag, wizard answer, card-flow value), stored on the row. Existing rows are never re-resolved; declarations are consulted only at creation, except threading, which stays live via `messaging_group_agents.threads` (`NULL` = inherit).

All creation paths (`area51 wirings`/`messaging-groups`, `setup/register.ts`, the router's auto-create, the channel-approval card flow, the bootstrap scripts `scripts/init-first-agent.ts` / `scripts/init-cli-agent.ts`) go through the shared helpers in `src/channels/channel-defaults.ts`, so a platform's defaults are declared once and apply everywhere.
...
| `setup/templates.ts` | Template discovery + first-agent stamping through `area51 groups create --template` |
