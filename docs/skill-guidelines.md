...
Every reach-in with a **functional consequence** gets a test that goes **red if the wiring is deleted or drifts**. That's what protects custom installs from product updates. The tests are also the verification: there is no separate "verify" step.
...
- **Recipe entry**: how it composes with the install's other skills (ordering, dependencies).
...
- **Optional, the creator's call:** unit tests of the skill's own internal logic, or its behavior against an external service. Often good practice; not what defines a maintainable skill, because they don't protect against product update drift.
