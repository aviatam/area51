...
│  │    • mcp__area51__* (scheduler tools via IPC)                │    │
│  │                                                                │    │
│  └──────────────────────────────────────────────────────────────┘    │
│                                                                       │
└───────────────────────────────────────────────────────────────────────┘
...
The core ships with no channels built in — each channel (WhatsApp, Telegram, Slack, Discord, Gmail) is installed as a [Claude Code skill](https://code.claude.com/docs/en/skills) that adds the channel code to your Area51 install. Channels self-register at startup; installed channels with missing credentials emit a WARN log and are skipped.
...
│   ├── area51.log               # Host stdout
│   └── area51.error.log         # Host stderr
│   # Note: Per-container logs are in groups/{folder}/logs/container-*.log
│
└── launchd/
    └── com.area51.plist         # macOS service configuration
```
...
export const CONTAINER_IMAGE = process.env.CONTAINER_IMAGE || 'area51-agent:latest';
...
- `{{PROJECT_ROOT}}` - Absolute path to your area51 installation
...
Area51 uses a hierarchical memory system based on CLAUDE.md files.
...
Area51 has a built-in scheduler that runs tasks as full agents in their group's context.
...
Claude: [runs area51 tasks create --prompt "Send a reminder to review weekly metrics. Be encouraging!" --process-after "2024-02-05T09:00:00" --recurrence "0 9 * * 1"]
...
Claude: [runs area51 tasks create --prompt "Search for today's emails, summarize the important ones, and send the summary to the group." --process-after "2024-01-31T17:00:00"]
```
...
The `area51` MCP server is created dynamically per agent call with the current group's context. Scheduled task management lives in `area51 tasks`, not MCP.
...
Area51 runs as a single macOS launchd service.

### Startup Sequence

When Area51 starts, it:

1. **Ensures container runtime is running** - Automatically starts it if needed; kills orphaned Area51 containers from previous runs
...
<dict>
    ...
    <string>{{PROJECT_ROOT}}/logs/area51.log</string>
    <key>StandardErrorPath</key>
    <string>{{PROJECT_ROOT}}/logs/area51.error.log</string>
</dict>
</plist>
```
...
```bash
# Install service
cp launchd/com.area51.plist ~/Library/LaunchAgents/
...
# Check status
launchctl list | grep area51

# View logs
tail -f logs/area51.log
```
...
| ---------------------------------------- | --------------------------------- | ---------------------------------------------------------------------------------------- | ------------ |
| No response to messages | Service not running | Check `launchctl list                                                                    | grep area51` |
| "Claude Code process exited with code 1" | Container runtime failed to start | Check logs; Area51 auto-starts container runtime but may fail |
...
- `logs/area51.log` - stdout
- `logs/area51.error.log` - stderr
