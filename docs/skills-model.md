...
How Area51 stays customizable without breaking custom installs. This is the full version; [customizing.md](customizing.md) is the short one, and [skill-guidelines.md](skill-guidelines.md) is the authoritative checklist for writing a skill.

## The problem

People clone Area51 and change the code. When we ship updates, their changes can collide with ours and `git merge` turns into a fight. The more someone customized, the worse it gets. We can't grow the core without breaking custom installs unless the customization model is explicit.
...
This works for any install: a personal setup with three tweaks, a company build with fifty.

## An install is a recipe of skills
...
So an install is defined by its recipe. Most upgrades don't need to run it (see "Upgrading"), but it's what lets you rebuild the install from scratch on clean Area51, and it's how you hand your whole setup to someone else. It replaces every "what did I change" artifact you'd otherwise keep (a migration guide, a manifest, a pile of notes) with one runnable thing.

The recipe is the one install-specific thing. It lives in your checkout, not in the shared core. (A recipe is itself a skill: a SKILL.md listing the install's skills in apply order.)
...
The point isn't to slow you down at edit time. It's that nothing counts as part of your install until it's a skill, because that's the only form that survives an upgrade.
...
Two different moves, two different rules. Your **install pulls Area51 updates**: that's a normal pull, run by the update command, and it's safe precisely because your changes live beside the core as skills rather than inside it. A **skill never merges**: it installs by fetching files and copying them in. If a skill's instructions say `git merge`, it isn't built to this model.

The update takes one of two paths:

**Normal upgrade: pull and fix what breaks.** Most of the time it pulls the latest Area51 update, resolves the occasional small conflict, runs the tests, and fixes whatever they flag. This stays cheap _because_ the changes are small self-contained skills with tests: conflicts are rare, and when something does break, the failing test points at the exact skill and the fix is local.

**Rebuild from the recipe: the rare path.** Take fresh Area51 and apply every skill from scratch. The command flags this when you've fallen far behind across many breaking changes (a clean rebuild beats catching up step by step). It's also how you hand your entire install to someone else.
...
6. **Test against real installs.** Every core change and migration runs against a fleet of real, skill-built installs before shipping. Real proof on real installs.
...
The rule: **every skill is reviewed and approved before it goes into the public registry, and every new version is re-reviewed.** Approving once and trusting forever is how supply chains get poisoned. Automated checks (linting against the guidelines, plus a harness that applies the skill on fresh Area51, runs its tests, removes it, and applies it twice) will clear the mechanical part so human review can focus on intent and safety. First-party skills are trusted by where they come from; the gate is for the public registry.
