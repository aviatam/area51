...
Area51 talks to the OneCLI gateway (credential vault + egress proxy) through `@onecli-sh/sdk`. The gateway is an external component with its own release line, so Area51 pins the **sanctioned gateway version** in [`versions.json`](../versions.json) under `onecli-gateway`. When an update moves that pin, the gateway must be upgraded — this doc is the migration path. It is written to be handed to a coding agent verbatim: detect → upgrade → verify → rollback.

There is deliberately **no runtime version check, and setup does not migrate the gateway for you**: the gateway is a separate out-of-band component, and the migrator is your coding agent running `/update-area51` — it diffs `versions.json` across the update and routes you here when the `onecli-gateway` pin moved. (Setup detects a pre-`/v1` gateway and points at this doc, but never upgrades it.) Run the steps below verbatim.
...
**Remote gateway** — run the same command on the gateway's host (Area51 can't reach it over SSH).
...
Finally, restart the Area51 service (per-install names — derive with `setup/lib/install-slug.sh`):
...
If the Area51 update itself is being rolled back, also pin `@onecli-sh/sdk` back to its previous version in `package.json` and run `pnpm install`. Vault data is unaffected in both directions.
...
To roll back, run the same block after reverting `versions.json` (or checking out the previous Area51 version). The CLI is stateless — vault data lives in the gateway, so swapping the binary in either direction loses nothing.
