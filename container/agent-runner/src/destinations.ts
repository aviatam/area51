...
function buildDestinationsSection(mode: SessionMode): string {
  ...
  if (mode.kind === 'task') {
    lines.push(
      ...
      '',
      `Your final output is not sent to the user. End with a concise work-log summary. It is recorded automatically in \`tasks/${mode.taskId}.md\`. Read that file when you need context from earlier runs. Use \`area51 tasks append-log --msg "…"\` only for optional mid-run notes.`,
    );
    return lines.join('\n');
  }
